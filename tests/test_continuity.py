...
def test_continuity_pack_is_bounded_trust_labeled_and_writes_ledgers(tmp_path: Path) -> None:
    ...
    assert all(item.memory_type in {"semantic", "procedural", "resource", "rule", "episodic", None} for item in pack.items)
...
def test_continuity_pack_can_merge_carry_state_with_durable_memory(tmp_path: Path) -> None:
    memory_cell = init_cell(tmp_path, "memory-cell", cell_type="memory")
    continuity_cell = init_cell(tmp_path, "continuity-cell", cell_type="continuity")
    ...
    capture_live_context(
        LiveContextCaptureRequest(
            ...
            task_id="task-carry",
            entry_kind="goal",
            content="Resume the typed carry-state flow.",
    ...
    pack = assemble_continuity_pack(
        ContinuityPackRequest(
            ...
            runtime_id="synthetic-runtime",
            session_id="session-carry",
            compaction_id="cmp-carry",
            query="typed carry-state flow",
            mode="advisory",
            max_items=4,
            max_tokens=120,
            write=True,
    ...
    assert pack.carry_state is not None
    assert pack.diagnostics["carry_candidate_count"] > 0
    assert any(item.provenance.get("carry_state") for item in pack.items)
    assert any(item.memory_id in memory_ids for item in pack.items)
